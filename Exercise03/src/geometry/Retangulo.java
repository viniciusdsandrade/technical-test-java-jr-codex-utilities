...
public class Retangulo implements Cloneable {
    ...
    // Metodo para calcular a área de interseção entre dois retângulos
    public static int areaOfIntersection(Retangulo r1, Retangulo r2) {
        ...
        // Aqui, também aceitamos interseções unidimensionais (como interseção em um ponto)
        if (interLeft <= interRight && interBottom <= interTop) {
            // Inclui os pontos no cálculo da largura e altura (quebra da regra matemática)
            interWidth = interRight - interLeft + 1;  // Adiciona 1 para incluir os pontos
            interHeight = interTop - interBottom + 1; // Adiciona 1 para incluir os pontos
            return interWidth * interHeight;  // Retorna a área de interseção
        }
